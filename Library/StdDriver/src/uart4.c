...
void SMC2_ISR(void) interrupt 23          // Vector @  0x9B
{
 ...
 /* Since only enable receive interrupt, not add flag check */
		if(uart4_receive_flag == 0) 
		{
			uart4_receive_flag = 1;
//			P35 = ~P35;                     // GPIO1 toggle when interrupt			
			Current_TX4_Num = Save_TX_Num;
			TxBuff[Current_TX4_Num][0] = CH_0;
			TxBuff[Current_TX4_Num][2] = SC2DR;		
		...
		else
		{
			...
			TxBuff[Current_TX4_Num][TX4_Data] = SC2DR;

//			P35 = ~P35;                     // GPIO1 toggle when interrupt
			TX4_Data++;
			if(TX4_Data > BuffData_MAX)  
			{
				TX4_Data--;							
				BuffData4_OverFlg = 1;
			}	
